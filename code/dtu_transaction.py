...
class DownlinkTransaction(Singleton):
    ...
    def downlink_main(self, *args, **kwargs):
        ...
        # >>> 纯透传模式，直接串口返回
        # if cloud_type == "tcp_private_cloud" or TRANSPARENT_MODE:
        #     packed_data = data
        # else:
...
class OtaTransaction(Singleton):
    ...
    def event_ota_plain(self, *args, **kwargs):
        ...
        if current_settings["system_config"]["cloud"] == "mqtt_private_cloud":

            # kwargs:
            # {
            #     'topic': 'update_plan_topic',
            #     'data': {
            #         'file_checksum': '56',
            #         'type': 'lte',
            ...
            type = data.get('type')
            url = data.get('url')
            new_ver = data.get('new_ver')

            if type == 'lte':
                # TODO: 模组固件升级计划
                if new_ver != DEVICE_FIRMWARE_VERSION:
                    self.__remote_ota_action(action=1, module=None)
            elif type == 'mcu':
                # TODO: 设备固件升级计划
                self.__remote_ota_action(action=1, module=None)
        else:
...
class UplinkTransaction(Singleton):
    ...
    def __parse(self):
        ...
        """
        if TRANSPARENT_MODE:
            # pub_topic_id_list = list(self.__get_pub_topic_id_list())
    ...
    def start_uplink_main(self):
        ...
        self.__uplink_main_thread_id = _thread.start_new_thread(self.uplink_main, ())
        log.info('start new up_transaction uplink main thread.')

    def stop_uplink_main(self):
        log.info('set up_transaction stop flag to true.')
        self.STOP_FLAG = True
